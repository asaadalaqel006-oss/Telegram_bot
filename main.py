...
auto_reply_map = {normalize_text(key): value for key, value in build_auto_reply_keywords().items()}
...
async def send_auto_reply_if_needed(update: Update) -> bool:
    message = update.effective_message
    if not message:
        return False
    ...
    reply_text = auto_reply_map.get(normalized)
    if not reply_text:
        return False

    try:
        await message.reply_text(reply_text, disable_web_page_preview=True)
        logger.info("Sent auto reply for trigger=%s", normalized)
        return True
    except Exception as exc:
        logger.warning("Failed to send auto reply for trigger=%s: %s", normalized, exc)
        return False
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_message:
        await update.effective_message.reply_text(
            "البوت يعمل ✅\n"
            "وظيفته حذف الإعلانات والروابط وأرقام الهواتف، مع حظر مرسلي الإعلانات تلقائيًا، والرد التلقائي على الكلمات المحددة."
        )
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_message:
        await update.effective_message.reply_text(
            ...
            f"- عدد الردود التلقائية: {len(auto_reply_map)}\n"
...
async def moderate_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not MODERATION_ENABLED:
        await send_auto_reply_if_needed(update)
        return
    ...
    auto_replied = await send_auto_reply_if_needed(update)
    if auto_replied:
        return
...
def main() -> None:
    logger.info(
        "Starting Telegram moderation bot | moderation=%s links=%s phones=%s auto_replies=%s ad_phrases=%s abuse_terms=%s",
        MODERATION_ENABLED,
        BLOCK_LINKS,
        BLOCK_PHONE_NUMBERS,
        len(auto_reply_map),
